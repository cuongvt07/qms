...
<div class="field"><label>Công ty cung cấp</label><input id="pSupplier"></div><div class="field"><label>Hạn hóa chất / vật tư</label><input id="pExpiry" type="date"></div>
...
function openProductForm(id=''){const p=state.products.find(x=>x.id===id);$('productTitle').textContent=p?'Cập nhật sản phẩm':'Thêm sản phẩm';$('pId').value=p?.id||'';$('pCode').value=p?.code||'';$('pName').value=p?.name||'';$('pUnit').value=p?.unit||'';$('pPacking').value=p?.packing||'';$('pSupplier').value=p?.supplier||'';$('pExpiry').value=p?.expiry||'';$('pMin').value=p?.min||0;$('pMax').value=p?.max||0;show('productModal')}
function saveProduct(){const code=$('pCode').value.trim();if(!code||!$('pName').value.trim()||!$('pUnit').value.trim())return toast('Vui lòng nhập đủ mã, tên và đơn vị tính');const id=$('pId').value||uid();const existing=state.products.find(x=>x.id===id);if(!existing&&state.products.some(x=>x.code.toLowerCase()===code.toLowerCase()))return toast('Mã hàng "'+code+'" đã có thẻ kho');const cardNo=existing?.cardNo||nextCardNo();const obj={id,cardNo,code,name:$('pName').value.trim(),unit:$('pUnit').value.trim(),packing:$('pPacking').value.trim(),supplier:$('pSupplier').value.trim(),expiry:$('pExpiry').value||'',min:Number($('pMin').value||0),max:Number($('pMax').value||0)};const i=state.products.findIndex(x=>x.id===id);i>=0?state.products[i]=obj:state.products.push(obj);save();closeModal('productModal');render();toast(existing?'Đã cập nhật thẻ kho':'Đã tạo mã hàng & tự sinh thẻ kho '+cardNo)}
...
function renderSheet(){
  ...
  $('cardSheet').innerHTML=`<h3>THẺ KHO</h3>
   ...
   <div class="qc-info">
     ...
     <div class="qc-line"><label>Hạn hóa chất / vật tư:</label><span>${fmtDate(p.expiry)||'—'}</span></div>
