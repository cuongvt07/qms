...
function filtered(){
  const q=$('q').value.trim().toLowerCase(),g=$('fGroup').value,st=$('fStatus').value,ac=$('fActive').value;
  return DATA.items.filter(x=>
    (!q||(x.code+' '+x.name+' '+x.supplier).toLowerCase().includes(q))&&
    (!g||x.group===g)&&(!st||x.status===st)&&(ac===''||String(x.active?1:0)===ac));
}
function render(){
  const rows=filtered();
  $('tbody').innerHTML=rows.length?rows.map(x=>{const s=ST[x.status];return `<tr>
    ...
    :`<tr><td colspan="12" class="empty">Không có mã hàng phù hợp.</td></tr>`;
...
function clearFilters(){$('q').value='';$('fGroup').value='';$('fStatus').value='';$('fActive').value='';render()}
...
function openItem(id=''){
  ...
  $('iSupplier').value=x?.supplier||'';$('iMin').value=x?.min??0;$('iMax').value=x?.max??0;
...
function exportCsv(){
  const rows=[['Mã hàng','Tên hàng hóa','Nhóm hàng','ĐVT','Quy cách','Nhà cung cấp','Tồn','Min','Max','Trạng thái tồn','Số thẻ kho','Phát sinh','Trạng thái'],
    ...filtered().map(x=>[x.code,x.name,x.group,x.unit,x.packing,x.supplier,x.balance,x.min,x.max,ST[x.status][1],x.cardNo,x.txCount,x.active?'Đang dùng':'Ngừng dùng'])];
  const csv='﻿'+rows.map(r=>r.map(v=>`"${String(v??'').replace(/"/g,'""')}"`).join(',')).join('\n');
  const a=document.createElement('a');a.href=URL.createObjectURL(new Blob([csv],{type:'text/csv;charset=utf-8'}));
  a.download='danh-muc-ma-hang.csv';a.click();URL.revokeObjectURL(a.href);toast('Đã xuất danh mục')
}
['q','fGroup','fStatus','fActive'].forEach(id=>$(id).addEventListener(id==='q'?'input':'change',render));
