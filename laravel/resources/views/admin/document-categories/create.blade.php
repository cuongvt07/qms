<x-layouts.app>
    <x-slot name="title">Thêm mục tài liệu</x-slot>

    <div class="max-w-2xl">
        <div class="flex items-center gap-2 mb-6">
            <a href="{{ route('admin.document-categories.index') }}" class="text-sm text-gray-500 hover:text-gray-700">← Quay lại</a>
        </div>
        <h1 class="text-2xl font-bold text-gray-800 mb-6">Thêm mục tài liệu (TL)</h1>

        <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
            <form method="POST" action="{{ route('admin.document-categories.store') }}" class="space-y-5">
                @csrf

                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1">Tên mục <span class="text-red-500">*</span></label>
                    <input type="text" name="ten_muc" value="{{ old('ten_muc') }}" required
                ...
                <div>
                    ...
                    <textarea name="mo_ta" rows="3"
                              class="w-full rounded-md border-gray-300 shadow-sm focus:border-teal-500 focus:ring-teal-500 text-sm">{{ old('mo_ta') }}</textarea>
